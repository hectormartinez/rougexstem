...
python2.6 preprocess/main.py --output $OUTPUT --docpath $DOCS --manpath $REF --task u09 --reload --splitta-model preprocess/splitta/model_nb/

for i in $OUTPUT/*.sent ; do preprocess/penn_treebank_tokenizer.sed $i > $i.tok;done

export HOSTNAME=localhost
python2.6 summarizer/inference.py -i $OUTPUT -o $OUTPUT -t u09 --manpath $REF --decoder glpsolve
rm tmp_decoder.*
